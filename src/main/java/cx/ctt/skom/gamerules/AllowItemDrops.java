...
public class AllowItemDrops implements Gamerule {
    final static Consumer<PickupItemEvent> pickup = event -> {
    ...
    final static Consumer<ItemDropEvent> drop = event -> {
        ...
        Vec velocity = player.getPosition().direction().mul(6*5);
    ...
    public static void register(GlobalEventHandler handler, int droppedItemLifetime) {
        ...
        handler.addListener(ItemDropEvent.class, event -> drop.accept(event));
        handler.addListener(PickupItemEvent.class, event -> pickup.accept(event));
