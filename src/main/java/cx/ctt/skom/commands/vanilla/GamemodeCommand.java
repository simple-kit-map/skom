package cx.ctt.skom.commands.vanilla; // literal copy paste of https://github.com/Minestom/VanillaReimplementation/blob/master/commands/src/main/java/net/minestom/vanilla/commands/GamemodeCommand.java
...
public class GamemodeCommand extends Command {
    ...
    public static void setUpTheSneakyGamemodeAliases(GlobalEventHandler globalEventHandler) {
        // very sneaky
    ...
    public GamemodeCommand() {
        ...
        ArgumentEnum<@NotNull GameMode> gamemode = ArgumentType.Enum("gamemode", GameMode.class).setFormat(ArgumentEnum.Format.LOWER_CASED);
        ...
        ArgumentInteger argumentIntegerArg = ArgumentType.Integer("gamemode");
        addSyntax((sender, context) -> {
            //Limit execution to players only
            if (!(sender instanceof Player playerSender)) {
                sender.sendMessage(Component.text("Please run this command in-game.", NamedTextColor.RED));
            ...
            int gamemodeLevel = context.get(argumentIntegerArg);
            GameMode mode = intToGamemode(gamemodeLevel);
            if (mode == null){
                sender.sendMessage(Component.text("Invalid gamemode level " + gamemodeLevel, NamedTextColor.RED));
                return;
            }
            executeSelf(playerSender, mode);
        }, argumentIntegerArg);
